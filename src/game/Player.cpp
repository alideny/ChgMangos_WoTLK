    ...
    if (GetMapId() != mapid)
    {
        if (!(options & TELE_TO_CHECKED))
        {
            if (!CheckTransferPossibility(mapid))
            {
                ...
                DEBUG_LOG("角色:TeleportTo %s is NOT teleported to map %u (requirements check failed)", GetName(), mapid);
        ...
        DEBUG_LOG("角色:TeleportTo %s is being far teleported to map %u", GetName(), mapid);
    }
    else
    {
        DEBUG_LOG("角色:TeleportTo %s is being near teleported to map %u", GetName(), mapid);
    }
...
void Player::DeleteFromDB(ObjectGuid playerguid, uint32 accountId, bool updateRealmChars, bool deleteFinally)
{
    ...
    switch(charDelete_method)
    {
        ...
        case 0:
        {
            ...
            CharacterDatabase.PExecute("DELETE FROM armory_character_feed_log WHERE guid = '%u'", lowguid);
...
{
    sLog.outString("角色: 清除已删除 %u 天的角色数据...", keepDays);
    ...
    if (resultChars)
    {
        sLog.outString("角色: 找到 %u 个待删除角色...",uint32(resultChars->GetRowCount()));
        do
        {
...
void Player::CleanupChannels()
{
    ...
    DEBUG_LOG("玩家: channels cleaned up!");
}
...
void Player::CheckAreaExploreAndOutdoor()
{
    ...
    if (!(currFields & val))
    {
        ...
        if(!p)
        {
            sLog.outError("玩家: Player %u discovered unknown area (x: %f y: %f map: %u", GetGUIDLow(), GetPositionX(),GetPositionY(),GetMapId());
        }
        else if (p->area_level > 0)
        {
            ...
            DETAIL_LOG("玩家: Player %u discovered a new area: %u", GetGUIDLow(), area);
        }
    }
}
...
bool Player::LoadFromDB(ObjectGuid guid, SqlQueryHolder *holder )
{
    ...
    {
        sLog.outError("角色:LoadFromDB player %s have invalid coordinates (map: %u X: %f Y: %f Z: %f O: %f). Teleport to default race/class locations.",
            ...
            if (gt)
            {
                ...
                if (at)
                {
                    if (CheckTransferPossibility(at))
                    {
                        ...
                        sLog.outError("角色:LoadFromDB %s try logged to instance (map: %u, difficulty %u), but transfer to map impossible. This _might_ be an exploit attempt.", GetObjectGuid().GetString().c_str(), savedLocation.mapid, GetDifficulty());
                }
                else
                    sLog.outError("角色:LoadFromDB %s logged in to a reset instance (map: %u, difficulty %u) and there is no area-trigger leading to this map. Thus he can't be ported back to the entrance. This _might_ be an exploit attempt.", GetObjectGuid().GetString().c_str(), savedLocation.mapid, GetDifficulty());
            }
            else
            {
                transGUID = 0;
...
void Player::_LoadAuras(QueryResult *result, uint32 timediff)
{
    ...
    if (result)
    {
        do
        {
            ...
            {
                sLog.outError("角色:LoadAuras Unknown caster %u, ignore.",fields[0].GetUInt64());
                continue;
            }
...
void Player::SaveToDB()
{
    ...
    if (sWorld.getConfig(CONFIG_BOOL_ARMORY_ENABLE))
    {
        ...
        for (WowarmoryFeeds::iterator iter = m_wowarmory_feeds.begin(); iter < m_wowarmory_feeds.end(); ++iter) {
            sWowarmory << "INSERT IGNORE INTO armory_character_feed_log (guid,type,data,date,counter,difficulty,item_guid,item_quality) VALUES ";
...
void Player::UpdateHomebindTime(uint32 time)
{
    ...
    else
    {
        ...
        data << uint32(ERR_RAID_GROUP_NONE);                // error used only when timer = 0
        GetSession()->SendPacket(&data);
        DEBUG_LOG("玩家: Player '%s' (GUID: %u) will be teleported to homebind in 60 seconds", GetName(),GetGUIDLow());
    }
}
...
void Player::UnsummonPetTemporaryIfAny(bool full)
{
    ...
    for (GroupPetList::const_iterator itr = m_groupPetsTmp.begin(); itr != m_groupPetsTmp.end(); ++itr)
    {
        if (Pet* pet = petmap->GetPet(*itr))
        {
            ...
            DEBUG_LOG("角色:UnsummonPetTemporaryIfAny tempusummon pet %s ",(*itr).GetString().c_str());
        }
    }
...
float Player::GetSpellDamageMod()
{
    if (((Player*)this)->GetTeam() == ALLIANCE)
...
bool Player::CheckTransferPossibility(uint32 mapId)
{
    ...
    if (!targetMapEntry)
    {
        sLog.outError("角色:CheckTransferPossibility: player %s try teleport to map %u, but map not exists!", GetObjectGuid().GetString().c_str(), mapId);
        return false;
    }
    ...
    if (!at)
    {
        if (isGameMaster())
        {
            sLog.outDetail("角色:CheckTransferPossibility: gamemaster %s try teleport to map %u, but entrance trigger not exists (possible for some test maps).", GetObjectGuid().GetString().c_str(), mapId);
            return true;
        }
        ...
        if (targetMapEntry->IsContinent())
        {
            ...
            {
                sLog.outError("角色:CheckTransferPossibility: player %s try teleport to map %u, but not has sufficient expansion (%u instead of %u)", GetObjectGuid().GetString().c_str(), mapId, GetSession()->Expansion(), targetMapEntry->Expansion());
                return false;
            }
            return true;
        }
        sLog.outError("角色:CheckTransferPossibility: player %s try teleport to map %u, but entrance trigger not exists!", GetObjectGuid().GetString().c_str(), mapId);
        return false;
    }
...
bool Player::CheckTransferPossibility(AreaTrigger const*& at, bool b_onlyMainReq)
{
    ...
    DEBUG_LOG("角色:CheckTransferPossibility %s check lock status of map %u (difficulty %u), result is %u", GetObjectGuid().GetString().c_str(), at->target_mapId, GetDifficulty(targetMapEntry->IsRaid()), status);
    ...
    switch (status)
    {
        ...
        default:
            ...
            sLog.outError("角色:CheckTransferPossibility player %s has unhandled AreaLockStatus %u in map %u (difficulty %u), do nothing", GetObjectGuid().GetString().c_str(), status, at->target_mapId, GetDifficulty(targetMapEntry->IsRaid()));
            return false;
    }
...
uint32 Player::GetEquipGearScore(bool withBags, bool withBank)
{
    ...
    if (count)
    {
        ...
        DEBUG_LOG("玩家: calculating gear score for %u. Result is %u", GetObjectGuid().GetCounter(), res);
